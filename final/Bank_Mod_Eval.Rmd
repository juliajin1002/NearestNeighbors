...
Now use our model to predict a sample dataset.

```{r}
# use the model to reevaluate a random selection of our data
df <- data.frame(read.csv(file="bank1.csv"))
df[df=='no'] <- 0
df[df=='yes'] <- 1
df$housing <- as.factor(df$housing)
df$loan <- as.factor(df$loan)
# predict
df$prediction <- predict(mybanklog2b, newdata = df, type = "response" )
```

```{r}
df$p_class <- as.factor(ifelse(df$prediction>cutoff, 1, 0))
nrow(df[(df['y']==1) & (df['p_class']==0)])
```

For this sample we manage to reduce FN to 0.
