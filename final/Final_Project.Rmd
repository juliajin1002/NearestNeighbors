...
For these questions, our target variable is categorical: whether someone subscribed to the bank's product or not after engaging with an advertising campaign. Therefore, we cleaned, wrangled, and explored the bank's dataset and developed a logistic regression model to predict whether or not a customer subscribed to a bank term deposit. To deal with imbalanced responses from the observed marketing campaign, as there were many more 'No' than 'Yes' subscription responses, We also used a combination of undersampling and oversampling techniques in our model development. Lastly we analyze the best cutoff and minimized cost of our prediction, and explored the relative importance of each feature in our predictions. In this report we discuss in detail our methodology, results, and conclusions.
